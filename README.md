...
PostgreSQL поднимется на порту 5432, веб-интерфейс — на **http://localhost:8000**. Схема БД создаётся автоматически при первом запуске (миграции в `migrations/` монтируются в `docker-entrypoint-initdb.d`).
...
5. Откройте в браузере http://localhost:8000 и задайте вопрос, например:
...
- **Веб**: откройте http://localhost:8000, введите вопрос в поле и нажмите «Выполнить». Будет показан сгенерированный SQL и таблица результата.
